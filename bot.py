...
from core.strategies import signal_trend_fast, signal_range_fast
...
def main():
    ...
    symbol = cfg.get("symbol","BTC/EUR"); timeframe = cfg.get("timeframe","5m"); limit = int(cfg.get("ohlcv_limit",500))
    spend_pct = float(cfg.get("spend_pct",0.50)); min_trade_eur = float(cfg.get("min_trade_eur",6.0))
    A = cfg.get("strategy_A", {}); B = cfg.get("strategy_B", {})
    A_sl, A_tp = float(A.get("sl_atr",0.7)), float(A.get("tp_atr",1.1))
    B_sl, B_tp = float(B.get("sl_atr",0.6)), float(B.get("tp_atr",0.9))
    state["symbol"] = symbol; state["timeframe"] = timeframe; state["bot_status"] = "running"; save_state(state)

    acquire_lock(); send_telegram("🚀 KrakenBot FAST avviato. (5m, SL reale + TP software + /closepositionnow)")
    feed = DataFeed(); ex = KrakenExecutor()
    last_k = state.get("last_bar_key"); last_reg = state.get("regime"); attempt = 0
    print(f"LIVE FAST {symbol} tf={timeframe} | spend_pct={spend_pct:.2f} | min_trade={min_trade_eur:.2f}")
    ...
    while True:
        try:
            ...
            if FORCE_CLOSE_REQUEST and in_pos:
                ...
                IN_CRITICAL = False; save_state(state); time.sleep(10); continue
            ...
            k = bar_key(df)
            if k == last_k:
                save_state(state); time.sleep(20); continue
            ...
            if in_pos:
                state["last_event"] = "position_monitored"; save_state(state); time.sleep(10); continue
            if reg == "CHAOS":
                state["last_event"] = "chaos_skip"; save_state(state); time.sleep(20); continue

            if reg == "TREND":
                sig = signal_trend_fast(df); sl_m, tp_m, mode = A_sl, A_tp, "FAST/TREND"
            else:
                sig = signal_range_fast(df); sl_m, tp_m, mode = B_sl, B_tp, "FAST/RANGE"

            if ai_enabled():
                try: send_telegram(ai_recommendation({"symbol":symbol,"timeframe":timeframe,"regime":reg,"price":last,"signal":sig})[:3500])
                except Exception: pass

            if sig != "BUY":
                state["last_event"] = f"no_setup_{mode}"; save_state(state); time.sleep(20); continue

            eur_spend = min(eur_free * spend_pct, max(0.0, eur_free - 0.5))
            if eur_spend < min_trade_eur:
                state["last_event"] = "insufficient_funds"; save_state(state); send_telegram(f"⚠️ Setup {mode} ma EUR insufficienti (free={eur_free:.2f})."); time.sleep(20); continue

            qty = eur_spend / ask
            d2 = df.copy(); d2["atr_val"] = atr(d2)
            atr_v = safe_float(d2.iloc[-1]["atr_val"], 0.0)
            if atr_v <= 0:
                state["last_event"] = "invalid_atr"; save_state(state); time.sleep(20); continue
            ...
            if total_btc2 <= 1e-7:
                state["last_event"] = "buy_no_btc_visible"; save_state(state); send_telegram("⚠️ BUY fatto ma BTC non visibile (attendo)."); IN_CRITICAL = False; time.sleep(20); continue
            ...
            try:
                ...
                IN_CRITICAL = False; time.sleep(20); continue
            ...
            IN_CRITICAL = False; attempt = 0; save_state(state); time.sleep(10)
