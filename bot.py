...
def position_context(bal: dict, open_orders: list[dict]) -> dict:
...
def main():
    ...
    while True:
        try:
            ...
            eur_free = ctx["eur_free"]; eur_total = ctx["eur_total"]
            btc_free = ctx["btc_free"]; btc_total = ctx["btc_total"]
            ...
            state["eur_free"] = eur_free
            state["eur_total"] = eur_total
            state["btc_free"] = btc_free
            state["btc_total"] = btc_total
            state["open_sell_orders"] = len(sell_orders)
            ...
            if in_pos:
                state["last_event"] = "position_monitored"
                save_state(state)
                time.sleep(10)
                continue
