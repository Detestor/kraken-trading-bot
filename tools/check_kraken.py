...
def main():
    ...
    eur_f=safe_float(b.get("free",{}).get("EUR",0.0)); eur_t=safe_float(b.get("total",{}).get("EUR",0.0))
    btc_f=safe_float(b.get("free",{}).get("BTC",0.0)); btc_t=safe_float(b.get("total",{}).get("BTC",0.0))
    ...
    print(f"Equity stimata: {(eur_f+btc_f*last):.2f}")
    print("\n=== OPEN ORDERS ===")
