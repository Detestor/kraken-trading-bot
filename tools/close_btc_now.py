...
def main():
    ex = KrakenExecutor()
    b = ex.fetch_balance()
    btc_f = safe_float(b.get("free", {}).get("BTC", 0.0))
    btc_t = safe_float(b.get("total", {}).get("BTC", btc_f))
    ...
    if btc_t <= 1e-7:
        print("Niente BTC da chiudere.")
        return
    b2 = ex.fetch_balance()
    btc_use = safe_float(b2.get("free", {}).get("BTC", 0.0))
    if btc_use <= 1e-7:
        btc_use = safe_float(b2.get("total", {}).get("BTC", 0.0))
